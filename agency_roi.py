...
# Safely initialize session states to prevent compile key errors
...
# Reactively re-adjust defaults if user changes base selection
...
# Read responsive variables
is_inr = st.session_state.currency == "INR (₹)"
sym = "₹" if is_inr else "$"
plan_cost = 17999 if is_inr else 199
sdr_cost = 150000 if is_inr else 2000

# STATIC MULTILINE STRING - strictly NO "f" prefix to avoid keyframe bracket compile errors!
st.markdown("""
<style>
  ...
  /* VIP Highlight container styles */
  .plx-money-card {
    background: #0f0f0f;
...
# Define 4-step Outbound Agency Roadmap template
timeline_template = """
<div class="section">
...
<div class="timeline-container">
  ...
  <div class="timeline-block">
    ...
    <div class="timeline-pointer">
      <div class="plx-step-card">
        <div class="plx-step-tag" style="background:#e6fcf5; color:#087f5b;">🔍 Scanning</div>
        <div class="plx-step-title">Deep Web Intent Scanning</div>
        <div class="plx-step-desc">Platinux automatically scans millions of live websites 24/7, flagging broken designs, mobile layout issues, and outdated landing pages that need upgrading.</div>
        <div class="plx-notif">
          <div class="plx-notif-dot"></div>
          <div class="plx-notif-text"><strong>System Scan:</strong> 12,450 local B2B sites audited -> 1,000 flagged with high technical debt.</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="timeline-block">
    ...
    <div class="timeline-pointer">
      <div class="plx-step-card">
        <div class="plx-step-tag" style="background:#fff9db; color:#f08c00;">🎯 Filtering</div>
        <div class="plx-step-title">Sifting 1,000 to 100 Hot Leads</div>
        <div class="plx-step-desc">Instead of wasting team hours cold-outreaching blindly to 10,000 sites, Platinux's ML filters the technical flags down to the <strong>100 high-potential leads</strong> with the highest conversion probability.</div>
        <div class="plx-step-detail">Focuses outbound sales efforts purely on pre-screened targets, maximizing sales efficiency.</div>
      </div>
    </div>
  </div>
  ...
  <div class="timeline-block">
    ...
    <div class="timeline-pointer">
      <div class="plx-step-card">
        <div class="plx-step-tag" style="background:#e7f5ff; color:#1c7ed6;">📄 Auditing</div>
        <div class="plx-step-title">Automated Audit Reports</div>
        <div class="plx-step-desc">For each of the 100 narrowed leads, Platinux instantly generates a bespoke, stunning Website Audit Report highlighting real conversion friction. Direct to your outreach.</div>
        <div class="plx-step-detail">Agencies using custom audits in their outreach experience a **3× increase** in response rates.</div>
      </div>
    </div>
  </div>

  <!-- Step 4: Acquisition (VIP Callout Card) -->
  <div class="timeline-block">
    ...
    <div class="timeline-pointer">
      <div class="plx-step-card plx-money-card">
        <div class="plx-step-tag" style="background:rgba(0,196,140,.15); color:#00c48c;">💰 Acquisition</div>
        <div class="plx-step-title">Outreach & Close. Money in.</div>
        <div class="plx-step-desc">Your team sends value-first messages with the audit attached. Prospects see exactly what is broken, bypassing gatekeepers and filling your agency calendar.</div>
...
with calc_col_left:
    ...
    # Fixed Metric Alert (replacing standard lead slider widget)
    st.markdown("""
    ...
    # Project sizes linked to responsive keys
    if is_inr:
